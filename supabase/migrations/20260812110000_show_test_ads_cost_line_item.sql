-- Test ads spend was already excluded from ad_spend/other_expenses (see
-- 20260810113200_exclude_test_ads_from_bonus_calc.sql), so it correctly had
-- zero effect on net profit — but there was nowhere in the breakdown to
-- actually see the amount, unlike easy_order_cost which gets its own line.
-- Add a test_ads_cost figure that flows through the same pipeline as
-- easy_order_cost for display purposes only: it is never subtracted from
-- net profit (the company absorbs it, not the marketer), so `net` keeps
-- the exact same formula as before.

ALTER TABLE public.monthly_marketer_bonuses
  ADD COLUMN IF NOT EXISTS test_ads_cost numeric(14,2) NOT NULL DEFAULT 0;

-- 1) load_bonus_month_inputs: also report the test_ads total
...
-- 2) compute_bonus_figures: accept + pass through test_ads_cost, never
--    subtracted from `net` (that's the whole point — the company eats it)
CREATE OR REPLACE FUNCTION public.compute_bonus_figures(
  ...
  _test_ads_cost numeric DEFAULT 0
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
DECLARE
  ...
  test_ads numeric(14,2) := COALESCE(_test_ads_cost, 0);
  rate numeric(6,4) := 0;
  net numeric(14,2);
...
BEGIN
  ...
  -- test_ads is intentionally NOT part of this formula — the company
  -- absorbs it, so it must not touch the marketer's net profit or bonus.
...
-- 3) preview_monthly_bonus: pass test_ads_cost through
...
-- 4) calculate_monthly_bonus: pass through + persist test_ads_cost
...
BEGIN
  ...
  IF existing.id IS NULL THEN
    ...
    INSERT INTO public.monthly_marketer_bonuses (
      ...
      realized_commission, ad_spend, easy_order_cost, other_expenses, test_ads_cost, net_profit_before_bonus,
      ...
      (res->>'other_expenses')::numeric, (res->>'test_ads_cost')::numeric,
      (res->>'net_profit_before_bonus')::numeric,
  ...
  ELSE
    ...
    UPDATE public.monthly_marketer_bonuses SET
      ...
      test_ads_cost = (res->>'test_ads_cost')::numeric,
